...
geometry_msgs::Point uwb_out_posi;
...
int main (int argc, char** argv){
    ...
    ros::NodeHandle n;
    //subscribe to mocap
    //  ros::Subscriber ground_truth = n.subscribe("robot01", 1000, callback);
    //发布主题sensor
    ...
    while(ros::ok()){
        ...
        if((data_frame[0] == a) && (data_frame[1] == b))
        {
            ...
            if(data_frame_sum == data_frame[128-1])
            {
                ...
                pos.z = (float)Byte32(sint32 ,data_frame[12],data_frame[11],data_frame[10], 0) / 256000.0f;
                ...
                uwb_out.id = id;
                uwb_out.position.x = pos.x;
                uwb_out.position.y = pos.y;
                uwb_out.position.z = pos.z;
